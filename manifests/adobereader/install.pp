class forthewin::adobereader::install {
  ...
  if $forthewin::adobereader::verbose {
    ...
    info("[${trusted[certname]}] fullversion                = ${fullversion}")
  ...
  if ((($is_adobereaderdc_installed) and ($managed_version == 'DC')) or (($is_adobereaderxi_installed) and ($managed_version == 'XI'))) {
    ...
    if $forthewin::adobereader::verbose {
      info("[${trusted[certname]}] cmd_line                   = ${cmd_line}")
      info("[${trusted[certname]}] cmd_unless                 = ${cmd_unless}")
      info("[${trusted[certname]}] package_name               = ${package_name}")
      info("[${trusted[certname]}] source_file                = ${source_file}")
    }
